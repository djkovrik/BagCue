...
- QG-008: owner confirmation that the shared production endpoint is final, stable, operationally owned, and already used by the owner's applications, plus a public HTTPS smoke response with the documented schema, policy version, and expiry no later than 72 hours. Internal service implementation and operations are owner-managed external evidence rather than per-app release work. Client handling of protected/non-protected/unknown responses and all fail-closed error paths remains covered by QG-007 tests.
...
The owner confirmed completion of every `docs/CI-RELEASE-SETUP.md` item and every legal review on 2026-09-10. The stable canonical policy URL is `https://sedsoftware.com/apps/bagcue/policy.html`; the hosted bytes match `docs/bagcue-policy.html` at approved SHA-256 `3bace33ed5b83c21c0901d20f6fca23c622fb9d7f5402f498209b5e119566e7a`. This resolves setup/legal blockers for QG-003, QG-005, QG-009, QG-011, and QG-012. The owner also confirmed on 2026-09-10 that the shared privacy-region endpoint is final, stable, production-ready, operationally complete, and already used by the owner's applications; together with the current public HTTPS/schema/expiry smoke, this closes QG-008. Each new candidate still needs a current durable result on its other declared verification surfaces.
