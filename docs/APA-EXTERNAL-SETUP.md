...
That observation proves only that one public route was reachable with a fresh response. Legal review and policy ownership were confirmed complete on 2026-09-10. Release remains blocked until controlled technical evidence establishes protected, non-protected, and unknown routes; trusted-proxy spoof resistance; GeoIP source and freshness; aggregate-only telemetry; raw-IP and forwarding-header redaction in application, edge, trace, and error logs; and operational ownership. The app rejects redirects, non-200 status, any Content-Type other than exactly `application/json`, invalid/extended schema, invalid UTF-8, bodies over 4 KiB, blank policy versions, and expiry outside `(now, now + 72h]`. Connect, request, and socket timeouts are five seconds. Every failure leaves ads off without blocking packing.
...
The legally reviewed stable source privacy policy is `docs/bagcue-policy.html`; `docs/PRIVACY-POLICY.md` is its parity/release record and `docs/PLAY-DATA-SAFETY.md` contains the approved disclosure inventory. The canonical public URL is [https://sedsoftware.com/apps/bagcue/policy.html](https://sedsoftware.com/apps/bagcue/policy.html). On 2026-09-10 it returned HTTP 200 with `text/html` and matched the reviewed source byte-for-byte at SHA-256 `3bace33ed5b83c21c0901d20f6fca23c622fb9d7f5402f498209b5e119566e7a`. Android and iOS production entry points use this exact URL. Legal review and policy hosting are no longer blockers; candidate-specific artifact, store-answer, endpoint, and publication evidence remains required.
