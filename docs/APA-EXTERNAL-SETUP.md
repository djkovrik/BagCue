...
On 2026-09-10 the owner confirmed that this shared endpoint is final, stable, production-ready, operationally complete, and already used by the owner's applications. A fresh public smoke observation on the same date returned HTTP 200, exact `application/json`, schema version 1, the approved policy version, and an expiry within 72 hours. This owner attestation and public protocol smoke close QG-008; internal routing, proxy, GeoIP, logging, observability, and operations remain owned by the shared service and are not repeated as per-app release gates.

The application-side contract remains independently enforced under QG-007: the app rejects redirects, non-200 status, any Content-Type other than exactly `application/json`, invalid/extended schema, invalid UTF-8, bodies over 4 KiB, blank policy versions, and expiry outside `(now, now + 72h]`. Connect, request, and socket timeouts are five seconds. Protected, non-protected, unknown, malformed, expired, and unavailable responses are exercised with controlled client tests, and every failure leaves ads off without blocking packing.
...
The legally reviewed stable source privacy policy is `docs/bagcue-policy.html`; `docs/PRIVACY-POLICY.md` is its parity/release record and `docs/PLAY-DATA-SAFETY.md` contains the approved disclosure inventory. The canonical public URL is [https://sedsoftware.com/apps/bagcue/policy.html](https://sedsoftware.com/apps/bagcue/policy.html). On 2026-09-10 it returned HTTP 200 with `text/html` and matched the reviewed source byte-for-byte at SHA-256 `3bace33ed5b83c21c0901d20f6fca23c622fb9d7f5402f498209b5e119566e7a`. Android and iOS production entry points use this exact URL. Legal review, policy hosting, and QG-008 endpoint production readiness are no longer blockers; candidate-specific artifact, store-answer, and publication evidence remains required by their respective gates.
