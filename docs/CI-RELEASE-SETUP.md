...
- `PublishAndroidRelease.yml`: SemVer tag, manual retry, or reusable call. It checks out the exact tag, builds signed/shrunk APK+AAB, verifies signatures/mapping/invariants, uploads evidence, and publishes the accepted artifact directly to Google Play Production through protected environment `google-play-publishing`.
...
- [x] Create protected environment `google-play-publishing`; add required reviewers and allow only protected SemVer tags. Keep this track-neutral name for Production publication and any future track-policy change.
...
- [x] Publish an Internal-track candidate and complete physical-device acceptance before enabling direct Production publication. Owner-confirmed on 2026-09-12.
...
- [ ] Confirm that the dedicated release-automation service account has app-scoped permission to manage the Production track for BagCue. The previously confirmed Internal-release permission is not sufficient evidence for Production.
...
- [ ] After merging the Production-track change, run `Create Android release` with the intended bump and 20-500 character EN/RU notes. Verify the annotated tag, GitHub prerelease, signed APK/AAB, mapping, optional native symbols, and exact version mapping.
- [x] Confirm the accepted candidate was published to Internal testing and the publish job waited for `google-play-publishing` approval.
...
- [ ] Confirm the first Production publish job waited for `google-play-publishing` approval and completed successfully for the exact immutable tag.

## Production publication, retry, and rollback

After the Internal candidate and device matrix are accepted, `Publish Android release` sends the exact immutable tagged AAB to the Production track with status `completed`. The protected `google-play-publishing` environment is the final human approval boundary. Retry an external failure by manually invoking the workflow with the same existing tag. If bytes or source change, create a new SemVer tag/versionCode. Roll back by halting availability in Play Console where possible or publishing a higher-versionCode corrective build—never move or overwrite an existing tag.

## Reconciled setup state

The owner confirmed completion of every original setup checkbox in this guide on 2026-09-10, then confirmed on 2026-09-12 that an Internal-track release was created and tested on devices. The repository therefore treats the Internal-publication and device-acceptance prerequisites for enabling Production publication as completed for the accepted candidate. Production permission for the service account, the new immutable tag, and the first Production job remain explicit go-live checks. The production workflow remains protected by `google-play-publishing` approval.

Those confirmations are setup and candidate evidence, not a permanent PASS for future source revisions. Each changed release candidate must retain its own green CI run, signed artifact verification, policy/Data safety comparison, and physical-device acceptance evidence before the protected Production job is approved. The owner separately confirmed the shared privacy-region endpoint production-ready and operationally complete on 2026-09-10; the public protocol smoke and `docs/APA-EXTERNAL-SETUP.md` record close QG-008.
