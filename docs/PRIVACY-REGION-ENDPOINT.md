...
This document specifies a small public endpoint used by BagCue to decide whether to display the app-owned consent screen before Yandex Ads is initialized. It replaces local country heuristics based on locale, SIM, time zone or a bundled country list.
...
- `X-Privacy-Client: bagcue/<app-version>` — optional for observability only; never use it as an identifier.
